...
export const scoutCommand: BotSlashCommand = {
    ...
    try {
      ...
      if (!liveGame.inGame) {
        await interaction.editReply(
          `${riotId} is not currently in a game. Try again when you're in loading screen.`,
        );
        return;
      }
      ...
      if (error instanceof WinConAPIError) {
        const retrySuffix =
          error.retryAfter && error.status === 429
            ...
            : "";
        await interaction.editReply(
          `Scout failed (${error.status}): ${error.message}.${retrySuffix}`,
        );
        return;
      }
