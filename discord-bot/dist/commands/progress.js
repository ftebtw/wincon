...
export const progressCommand = {
    ...
    async execute(interaction, context) {
        ...
        try {
            ...
            const progressUrl = context.apiClient.websiteUrl(`/player/${toRiotSlug(riotId)}/progress`);
            ...
            if (error instanceof WinConAPIError) {
                const retrySuffix = error.retryAfter && error.status === 429
                    ...
                    : "";
                await interaction.editReply(`Progress lookup failed (${error.status}): ${error.message}.${retrySuffix}`);
                return;
            }
